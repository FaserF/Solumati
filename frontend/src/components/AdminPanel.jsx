...
import { Shield, Settings, Users, Save, RefreshCw, AlertTriangle, Check, UserX, ExternalLink, XCircle, ArrowLeft, Crown, UserMinus, Edit2, Mail } from 'lucide-react';
import { API_URL, APP_VERSION } from '../config';

const AdminPanel = ({ user, onLogout, onBack, t }) => {
    // Role Checks
    ...
    const executePunishment = async () => {
        ...
        if (punishModal.reportId) {
            await fetch(`${API_URL}/admin/reports/${punishModal.reportId}`, {
                method: 'DELETE',
                headers: authHeaders
            });
            fetchReports();
                    ...
                    <div className="bg-white rounded-xl shadow overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {users.length === 0 ? (
                                        <tr><td colSpan="6" className="p-8 text-center text-gray-400">{loading ? "Lade..." : t('admin.no_users')}</td></tr>
                                    ) : (
                                        users.map(u => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white p-6 rounded-xl shadow h-fit">
                            ...
                            <div className="space-y-4">
                                ...
                                {/* Guest Mode Toggle removed as requested. Logic now depends on User ID 0 active status. */}
                                <div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 border border-gray-200">
                        ...
                        <div className="mb-4">
                            ...
                            <textarea
                                className="w-full p-3 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-red-500"
                                placeholder="Details regarding the ban..."
                                value={customReason}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 border border-gray-200">
                        ...
                        <div className="mb-4">
                            ...
                            />
                            <p className="text-xs text-orange-500 mt-1">Warnung: Ändern der E-Mail setzt den Status auf "Unverifiziert".</p>
                        </div>
                        <div className="mb-6">
                            <label className="block text-sm font-bold text-gray-500 mb-2">{t('label.password')} (Reset)</label>
                            <input
                                className="w-full p-3 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
                            ...
                            <button onClick={saveUserEdit} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-blue-700 shadow-md transition transform active:scale-95">
                                {t('btn.save')}
                            </button>
                        </div>
                    </div>

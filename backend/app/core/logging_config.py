...
class ProgressFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if re.search(
            r"\d+[\.,]?\d*\s*(?:KB|MB|GB)\s*/\s*\d+[\.,]?\d*\s*(?:KB|MB|GB)", msg
        ) and re.search(r"/", msg):
            ...
            if len(size_matches) >= 2 and size_matches[0][0] == size_matches[-1][0]:
                ...
                keys_to_remove = [
                    k for k in _active_downloads.keys() if k[0] == record.name
                ]
        ...
        if re.search(
            r"hash.*(success|verified)|installer.*start|installer.*hash",
            msg,
            re.IGNORECASE,
        ):
            keys_to_remove = [

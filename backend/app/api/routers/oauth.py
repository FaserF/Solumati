...
async def process_oauth_login(
    user_info, provider: str, db: Session, link_user_id: Optional[int] = None
):
    """
    ...
    if link_user_id:
        target_user = (
            db.query(models.User).filter(models.User.id == link_user_id).first()
        )
        if not target_user:
        ...
        if existing_link:
            if existing_link.user_id == link_user_id:
                ...
                raise HTTPException(
                    400, f"This {provider} account is already linked to another user."
                )
    ...
    if user:
        ...
        db.commit()
        logger.info(
            f"Migrated legacy OAuth user {user.username} (ID {user.id}) to LinkedAccount {provider}"
        )
    ...
    # Create Link
    new_link = models.LinkedAccount(
        user_id=user.id, provider=provider, provider_user_id=provider_uid, email=email
    )
    db.add(new_link)
    db.commit()
...
async def github_login(state: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if not sso:
        raise HTTPException(501, "GitHub auth not configured")
...
async def github_callback(request: Request, db: Session = Depends(get_db)):
    ...
    return RedirectResponse(
        f"{APP_BASE_URL}/login?oauth_user={user.username}&oauth_id={user.id}&oauth_role={user.role}"
    )
...
async def google_login(state: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if not sso:
        raise HTTPException(501, "Google auth not configured")
    return await sso.get_login_redirect(state=state)
...
async def microsoft_login(state: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if not sso:
        raise HTTPException(501, "Microsoft auth not configured")
    return await sso.get_login_redirect(state=state)

...
def admin_update_user(
    ...
    if update.is_visible_in_matches is not None:
        user.is_visible_in_matches = update.is_visible_in_matches
    if update.two_factor_method is not None:
        user.two_factor_method = update.two_factor_method
    if update.role:
...
def admin_punish_user(
    user_id: int,
    action: schemas.AdminPunishAction,
    ...
    logger.info(
        f"Admin {current_admin.username} executing action {action.action} on user {user_id}"
    )
    ...
    if send_deactivation_email and action.action != "delete":
        background_tasks.add_task(
            send_account_deactivated_notification, user, deactivation_reason
        )
...
def admin_reset_2fa(
    ...
    logger.info(
        f"Admin {current_admin.username} reset 2FA for User {user.username} (ID: {user.id})"
    )
...
def get_admin_settings(
    ...
    support_page_conf = get_setting(
        db, "support_page", schemas.SupportPageConfig().dict()
    )
    reg_notify_conf = get_setting(
        db, "registration_notification", schemas.RegistrationNotificationConfig().dict()
    )
    ...
    if oauth_conf.get("github", {}).get("client_secret"):
        oauth_conf["github"]["client_secret"] = "******"
    if oauth_conf.get("google", {}).get("client_secret"):
        oauth_conf["google"]["client_secret"] = "******"
    if oauth_conf.get("microsoft", {}).get("client_secret"):
        oauth_conf["microsoft"]["client_secret"] = "******"

    # Mask CAPTCHA secret
    if captcha_conf.get("secret_key"):
        captcha_conf["secret_key"] = "******"
...
def update_admin_settings(
    ...
    save_setting(db, "support_page", settings.support_page.dict())
    save_setting(
        db, "registration_notification", settings.registration_notification.dict()
    )
    ...
    for provider in ["github", "google", "microsoft"]:
        ...
        if submitted_secret == "******":
            new_oauth[provider]["client_secret"] = current_oauth.get(provider, {}).get(
                "client_secret", ""
            )
...
def send_test_mail(
    ...
    from app.services.utils import create_html_email, send_mail_sync
...
def get_diagnostics(
            ...
            if response.status == 200:
                ...
                def parse_ver(v_str):
                    ...
                    return (
                        ...
                        base[1],
                        base[2],
                        ver_priority if is_beta else 4,
                        beta_num,
                    )
    ...
    return {
        ...
        "latest_beta_version": (
            latest_version_in_channel if update_channel != "stable" else None
...
def get_reports(
    ...
    for r in reports:
        res.append(
            schemas.ReportDisplay(
                id=r.id,
                reporter_username=(
                    r.reporter.username
                    if (r.reporter and r.reporter.username)
                    else "Unknown"
                ),
                reported_username=(
                    r.reported.username
                    if (r.reported and r.reported.username)
                    else "Unknown"
                ),
                reason=r.reason,
                status=r.status,

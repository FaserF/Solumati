...
from app.services.utils import (calculate_compatibility, create_html_email,
                                generate_unique_username, get_setting,
...
from app.services.export_service import (collect_user_data,
...
def create_user(
    ...
    if captcha_config.enabled:
        if not user.captcha_token:
            raise HTTPException(
                428, {"message": "CAPTCHA required", "captcha_required": True}
            )
    ...
    new_user = models.User(
        ...
        intent=user.intent,
        answers=json.dumps(
            user.answers if isinstance(user.answers, dict) else {}
        ),  # Store empty if list/none for now
        is_active=True,
    ...
    if reg_config.require_verification and not new_user.is_verified:
        ...
        content = t.get(
            ...
            "Welcome to Solumati! Please verify your email address.",
        )
...
def verify_email(id: int, code: str, db: Session = Depends(get_db)):
    ...
    if (
        ...
        or not secrets.compare_digest(code, user.verification_code)
    ):
...
def get_matches(user_id: int, db: Session = Depends(get_db)):
    ...
    if user_id != 0:
        ...
        if u.role not in ["admin", "moderator"]:
            if not is_profile_complete(u):
                ...
                raise HTTPException(
                    403, "Profile Incomplete. Please finish setting up your account."
                )
    ...
    if user_id == 0 or is_admin:
        query = query.filter(
            or_(models.User.is_visible_in_matches == True, models.User.role == "test")
        )
    else:
        # Normal users: Visible users only, AND NOT test users (double check)
        query = query.filter(
            models.User.is_visible_in_matches == True, models.User.role != "test"
        )
    ...
    for other in query.all():
        compatibility = calculate_compatibility(
            curr_answ, other.answers, curr_int, other.intent
        )
...
def discover_users(
    ...
    candidates = (
        db.query(models.User).filter(*base_filters, visibility_filter).limit(50).all()
    )
...
def update_profile(
    user_id: int, update: schemas.UserUpdate, db: Session = Depends(get_db)
):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
...
def update_account_settings(
    ...
    if user.role == "test":
        raise HTTPException(
            403, "Test users cannot change sensitive account settings (Email/Password)."
        )
...
async def export_user_data(
    ...
    if method == "email":
        ...
        if not mail_conf.get("enabled"):
            raise HTTPException(
                400, "Email service is not enabled. Please use download."
            )
    ...
    if method == "download":
        ...
        return StreamingResponse(
            iter([zip_buffer.read()]), media_type="application/zip", headers=headers
        )
        ...
        if zip_size > 12 * 1024 * 1024:
            raise HTTPException(
                413, "Export too large for email (Max 12MB). Please use download."
            )
        ...
        # Prepare content
        subject = (
            f"[{PROJECT_NAME}] {t.get('email.export.subject', 'Your Data Export')}"
        )
        ...
        def send_export_mail(target_email, zip_data, filename, subject, html_content):
            ...
            try:
                ...
                msg = MIMEMultipart(
                    "alternative"
                )  # Changed to alternative for HTML/Text
                msg["Subject"] = subject
                msg["From"] = (
                    f"{mail_conf.get('sender_name', 'Solumati')} <{mail_conf.get('from_email')}>"
                )
                msg["To"] = target_email
                ...
                with smtplib.SMTP(
                    mail_conf["smtp_host"], mail_conf["smtp_port"]
                ) as server:
                    if mail_conf.get("smtp_tls"):
                        server.starttls()
        ...
        background_tasks.add_task(
            ...
            user.email,
            zip_buffer.getvalue(),
            f"solumati_export_{user.username}.zip",
            subject,
            html_body,
        )
...
def request_password_reset(
    body: dict, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
    ...
    if user:
        ...
        if user.role == "admin":
            logger.warning(
                f"Password reset via mail blocked for admin: {identifier} (Use CLI tool)"
            )
            return {
                "status": "ok",
                "message": "If the account exists, a reset link has been sent.",
        ...
        if not user.email:
            logger.warning(
                f"Password reset blocked (no email) for user: {user.username}"
            )
            return {
                "status": "ok",
                "message": "If the account exists, a reset link has been sent.",
        ...
        content = t.get(
            "email.reset.content", "Click the button below to reset your password."
        )

...
async def fetch_dummy_image(client: httpx.AsyncClient, username: str) -> str:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(
                f"Failed to fetch AI image for {username}: Status {resp.status_code}"
            )
...
def ensure_guest_user(db: Session):
    try:
        ...
        if not guest:
            ...
            guest = models.User(
                id=0,
                email="guest@solumati.local",
                hashed_password=hash_password("NOPASSWORD"),
                real_name="Guest Explorer",
                username="Guest",
...
def ensure_admin_user(db: Session):
    try:
        ...
        if not admin:
            ...
            if admin_by_name:
                if admin_by_name.id != 1:
                    logger.warning(
                        f"Admin user exists but ID is {admin_by_name.id}, not 1. Skipping ID enforcement to prevent data corruption."
                    )
                else:
                    logger.info("Admin user exists with ID 1.")
            else:
                logger.info(
                    "No 'admin' user found. Creating initial admin account with ID 1..."
                )
        ...
        try:
            ...
            db.execute(
                text(
                    "SELECT setval('users_id_seq', GREATEST(10000, (SELECT MAX(id) FROM users)), true);"
                )
            )
            db.commit()
            ...
            logger.warning(
                f"Could not adjust sequence (might be first run or unsupported DB): {seq_e}"
            )
            db.rollback()
...
def check_emergency_reset(db: Session):
    ...
    try:
        ...
        if not user or user.role != "admin":
            logger.warning(
                f"Emergency reset requested for ID {target_id}, but user not found or not admin."
            )
            return
        ...
        db.query(models.SystemSetting).filter(
            models.SystemSetting.key == "admin_emergency_reset_target"
        ).delete()
...
def fix_dummy_user_roles(db: Session):
    ...
    try:
        ...
        if count > 0:
            db.commit()
            logger.info(
                f"Fixed role for {count} dummy users: {', '.join(fixed_names)} -> Set to 'test'."
            )
        else:
...
async def generate_dummy_data(db: Session):
    ...
    existing_test_users = (
        db.query(models.User).filter(models.User.role == "test").count()
    )
    if existing_test_users > 0:
        logger.info(
            f"TEST_MODE active but {existing_test_users} test users already exist, skipping dummy generation."
        )
        return
...
async def ensure_showcase_dummies(db: Session):
    ...
    try:
        ...
        if existing_count >= 5:
            logger.info(
                f"Showcase dummies exist ({existing_count}). Skipping generation."
            )
            return
        ...
        showcase_users = [
            {
                "name": "Alice",
                "intent": "longterm",
                "desc": "Looking for something serious.",
            },
            {
                "name": "Bob",
                "intent": "casual",
                "desc": "Just here to meet new people.",
            },
            {"name": "Charlie", "intent": "speeddate", "desc": "Fast and furious!"},
            {
                "name": "Diana",
                "intent": "friendship",
                "desc": "New in town, looking for friends.",
            },
